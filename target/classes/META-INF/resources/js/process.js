...
$(function(){
    ...
    $(grid_selector).jqGrid({
        ...
        colModel: [
            ...
            { label: '客户名称', name: 'client', width: 60 },
            { label: '完成时间', name: 'end_datetime', width: 60 },
            { label: '交付日期', name: 'delivery_date', width: 60 },
            { label: '急迫度', name: 'urgency', width: 60 }
        ],
    ...
    $("#modifyLearnBtn").click(function () {
        ...
        if(rows==0){
            ...
            task = "updateRecordMove";
            
            $("#id").val(data.id);
            $("#cs").val(data.mOVE);
            $("#bd").val(data.mOVE_DATE);
            $("#ed").val(data.mOVE_NAME);
            
            $('#myModalLabel').text('移动工单当前状态');
            $("#addModal").modal({
                keyboard : false,
                show : true,
                backdrop : "static"
